...
// use druid::im::Vector;
use druid::widget::{
    Axis, CrossAxisAlignment, Flex, Label, MainAxisAlignment, // Split, TabInfo, TabsPolicy, Button, RadioGroup, TextBox
    Tabs, TabsEdge, TabsTransition, ViewSwitcher, Painter
};
use druid::{AppLauncher, Color, FontFamily, FontStyle, FontWeight, Data, Lens, Widget, WidgetExt, WindowDesc, LinearGradient, UnitPoint}; // Env, theme
...
Python – Django – SQL – Heroku – Top Down 
"#;
...
AutoSar – ADD - Scrum - Process Elaboration - Outsourcing
"#;
...
Matlab - MicroChip - HMI 
"#;

const TEXT5: &str = r#"Former system engineer in electronic field, 
I decided to reorient me as developer for now two years. I made this 
challenging choice because during my past experiences, everything 
I loved the most was programming. Rich of those experiences, I am looking 
for innovative companies for enforced and enhanced my skills
...
const TEXT6: &str = r#"2012-2017
Master 2 RAM – Robotic Automatic & Mobility 
UVSQ – Paris Saclay 

Master 1 E3A - Electronics, Electric Energy & Automation
UVSQ - Paris Saclay

Licence 3 - Physics Theoric
UVSQ - Paris Saclay
...
pub fn main() {
    ...
    let main_window = WindowDesc::new(build_root_widget())
        .title("CV Merry de Lassus Saint Genies")
        .window_size((950.0, 470.0));
    ...
    let polka_dots = Painter::new(|ctx, _, _| {
        ...
        for y in 0..((bounds.height() / dot_diam).ceil() as usize) {
            for x in 0..((bounds.width() / dot_diam).ceil() as usize) {
                ...
                let circ = Circle::new((x, y), dot_diam );
                let purp = Color::rgb(0.37, 0.48, 0.90);
                ctx.fill(circ, &purp);
            }
        }
    });
...
merry2lassus@gmail.com
    ")
        ...
        .border(DARK_GREY, 4.0)
        .padding(10.0),
    ).with_child(Label::new("Personnal Skills
    •	Creativity 
    •	Leadership 
    •	Organization 
    •	Problem solving
...
fn build_tab_widget(tab_config: &TabConfig) -> impl Widget<AppState> {
    ...
    let main_tabs = Tabs::new()
        ...
        .with_tab("About me", Label::new(TEXT5))
        .with_tab("Education", Label::new(TEXT6))
        .with_tab_index(4);
    main_tabs
}
